...
namespace Backend.Repositories
{
        ...
        : IGameRepository
    {
        ...
        public Game CreateGame(int homeTeamID, int awayTeamID, string homeTeamName, string awayTeamName,
            int homeTeamScore, int awayTeamScore, int courtNumber, int overtimeCount, DateOnly date)
        {
            return executor.ExecuteNonQuery(
                new CreateGameDelegate(homeTeamID, awayTeamID, homeTeamName, awayTeamName,
                    homeTeamScore, awayTeamScore, courtNumber, overtimeCount, date));
        }
        ...
        public IReadOnlyList<Game> RetrieveGamesBySeason(int seasonID)
        {
            return executor.ExecuteReader(new RetrieveGamesBySeasonDelegate(seasonID));
        }

        private class RetrieveGamesBySeasonDelegate(int seasonId)
            : DataReaderDelegate<IReadOnlyList<Game>>("Basketball.RetrieveGamesBySeason")
        {
            public override void PrepareCommand(Command command) =>
                command.Parameters.AddWithValue("SeasonID", seasonId);

            public override IReadOnlyList<Game> Translate(Command command, IDataRowReader reader)
            {
                var games = new List<Game>();
                while (reader.Read()) games.Add(TranslateGame(reader));
                return games;
            }
        }
        ...
        private class CreateGameDelegate(
            int homeTeamID,
            int awayTeamID,
            string homeTeamName, // Added
            string awayTeamName, // Added
            int homeTeamScore,
            int awayTeamScore,
            int courtNumber,
            int overtimeCount,
            ...
            : NonQueryDataDelegate<Game>("Basketball.CreateGame")
        {
            ...
            public override Game Translate(Command command)
            {
                ...
                // Now passing all 10 required arguments
                return new Game(
                    gameID,
                    homeTeamID,
                    awayTeamID,
        ...
        private static Game TranslateGame(IDataRowReader reader)
        {
            return new Game(
                ...
                reader.GetString("HomeTeamName"), 
                reader.GetString("AwayTeamName"), 
                reader.GetInt32("HomeTeamScore"),
                reader.GetInt32("AwayTeamScore"),
                reader.GetInt32("CourtNumber"),
                reader.GetInt32("OvertimeCount"),
                DateOnly.FromDateTime(reader.GetDateTime("Date", DateTimeKind.Unspecified))
            );
        }
    }
}