...
-- HOME TEAM
SELECT
    p.PlayerID, g.GameID, p.TeamID,
    stats.playingTime, stats.turnovers, stats.rebounds, stats.assists,
    final.fgMade, final.fgTaken, final.tpMade, final.tpTaken,
    stats.fouls
FROM Basketball.Games g
JOIN Basketball.Players p ON p.TeamID = g.HomeTeamID
CROSS APPLY (
    SELECT
        ...
        -- FG raw
        (ABS(CHECKSUM(CAST(p.PlayerID AS BIGINT) * 1009 + CAST(g.GameID AS BIGINT) * 2003 + 6)) % 14) + 4 AS fgTaken,
        ABS(CHECKSUM(CAST(p.PlayerID AS BIGINT) * 5003 + CAST(g.GameID AS BIGINT) * 7001 + 7)) % 15 AS fgMadeRaw,
        -- 3PT raw
        ABS(CHECKSUM(CAST(p.PlayerID AS BIGINT) * 4007 + CAST(g.GameID AS BIGINT) * 6011 + 8)) % 5 AS tpTakenRaw,
        ABS(CHECKSUM(CAST(p.PlayerID AS BIGINT) * 8009 + CAST(g.GameID AS BIGINT) * 3001 + 9)) % 5 AS tpMadeRaw
) raw
CROSS APPLY (
    SELECT
        raw.fgTaken,
        raw.fgMadeRaw % (raw.fgTaken + 1) AS fgMade,
        raw.tpTakenRaw % (raw.fgTaken / 3 + 1) AS tpTaken,
        raw.tpMadeRaw % (raw.fgTaken / 3 + 1) AS tpMadeTemp
) calc
CROSS APPLY (
    SELECT
        calc.fgTaken,
        ...
        calc.tpTaken,
        CASE
            WHEN calc.tpMadeTemp > calc.fgMade THEN calc.fgMade
            WHEN calc.tpMadeTemp > calc.tpTaken THEN calc.tpTaken
            ELSE calc.tpMadeTemp
        END AS tpMade
) final
...
SELECT
    ...
    stats.fouls
FROM Basketball.Games g
JOIN Basketball.Players p ON p.TeamID = g.AwayTeamID
CROSS APPLY (
    SELECT
        ABS(CHECKSUM(CAST(p.PlayerID AS BIGINT) * 7919 + CAST(g.GameID AS BIGINT) * 6271 + 1)) % 30 + 5 AS playingTime,
        ABS(CHECKSUM(CAST(p.PlayerID AS BIGINT) * 3571 + CAST(g.GameID AS BIGINT) * 4969 + 2)) % 6 AS turnovers,
...
PRINT 'Totals: 4 Locations, 2 Leagues, 4 Seasons, 16 Teams, 52 Games, 80 Players, 520 PlayerGameStats rows.';
GO
